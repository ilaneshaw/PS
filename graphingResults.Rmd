...
coloursAllSame2 <- c("#3C6987FF", "#3C6987FF", "#3C6987FF",
                    "#3C6987FF", "#3C6987FF", "#3C6987FF")
...
sp.labs <- c("Alder Flycatcher", "Bay-breasted Warbler", "Black-capped Chickadee",
             ...
             "Gray Jay", "Least Flycatcher", "Mourning Warbler", 
...
#datasetWanted <- paste("birdDatasets$", spOfInterest, sep = "")
  ...
  ggtitle(paste("Kernel density plots for ", spFullName, " by forest or land cover class", sep = "")) +
    ...
    ylab("Prevalence of density value") +
...
assumptionsByClass2D_10 <- as.data.frame(read_csv(paste(outputsDir, "/outputBirdPreds/assumptionsByClass2D.csv", sep = "")))
...
assumptionsByClass$smoothingType <- as.factor(assumptionsByClass$smoothingType)
 #normality plot
   plotUnimodality <- ggplot(data = assumptionsByClass, 
    ...
    scale_y_continuous(breaks= pretty_breaks()) +
    theme_classic() + 
     scale_x_discrete(labels=c("1" = "1D", 
                               "2_10" = "2D (10 year\n age classes)", 
                               "2_20" = "2D (20 year\n age classes)",
                               "2_40" = "2D (40 year\n age classes)")) +  
    scale_fill_manual(values=coloursAllSame) +
...
plotSpearmanStats <- ggplot(data = spearmanStats,
  ...
  geom_boxplot(width=0.15, color="grey") +
    ggtitle("Correlation between BAM national models \nand mapped predictions, across bird species") +
    #xlab()  +
    scale_x_discrete(labels=c("spearman1D" = "1D", 
                              "spearman2D_10" = "2D (10 year\n age classes)",
                              "spearman2D_20" = "2D (20 year\n age classes)",
                              "spearman2D_40" = "2D (40 year\n age classes)")) +  
                               ylab("Spearman's \ncoefficient (p)") +
    scale_y_continuous(breaks= pretty_breaks()) +
    theme_classic() + 
    scale_fill_manual(values= coloursAllSame, 
...
plotAutocorStats <- ggplot(data = moranStats,
  ...
  geom_boxplot(width=0.15, color="grey") +
    ...
    ylab("Moran's I") +
    scale_y_continuous(breaks= pretty_breaks()) +
     scale_x_discrete(labels=c("moran1D" = "1D", 
                              "moran2D_10" = "2D (10 year\n age classes)",
                              "moran2D_20" = "2D (20 year\n age classes)",
                              "moran2D_40" = "2D (40 year\n age classes)")) +       
  theme_classic() + 
    scale_fill_manual(values= coloursAllSame, 
                      labels = c( "moran1D", "moran2D")) +
    theme(title = element_text(size = 12),
